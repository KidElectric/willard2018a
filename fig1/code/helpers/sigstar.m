function varargout=sigstar(groups,stats,nosort,alpha)
...
% the placement and drawing of the highlighting. 
% By default, Stars are drawn according to:
%   * represents p<=0.05
%  ** represents p<=1E-2
% *** represents p<=1E-3
%
% UPDATED 2014-08-30 - BI Added: alpha parameter which can indicate new
% alpha levels for star values (For Bonferonni corrections, for example).
% 
%
% Inputs
...
%          bar.
% ALPHA -  a vecotor of alpha values, in descending order of significance.
%          Length of 3. This will override the default values of star 
%          significance representations. By Default: alpha=[1E-3 1E-2 0.05];     
%
% Outputs
...
elseif length(alpha)>1 && alpha(end)< alpha(1)
    alpha=fliplr(alpha);
end
...
for ii=1:length(groups)
	...
	H(ii,:)=makeBar(xlocs(ii,:),thisY,stats(ii),alpha);
end
%-----------------------------------------------------
...
function H=makeBar(x,y,p,alpha)
...
elseif p<=alpha(3)
	stars='*';
elseif isnan(p) || p > alpha(3) %replaced: isnan(p) - BI
	stars='n.s.';
else
	stars='';
end
...
x=repmat(x,2,1);
y=repmat(y,4,1);
H(1)=plot(x(:),y,'-k','LineWidth',1.5,'UserData','Ex'); %BI added 'UserData','Ex'

%Increase offset between line and text if we will print "n.s."
%instead of a star. 
if p <= 0.05 %replaced: ~isnan(p) - BI
    offset=0.02;
    fs=18;
else
    offset=0.05;
...
H(2)=text(mean(x(:)),mean(y)+myRange(ylim)*offset,stars,...
   	...
   	'BackGroundColor','none','FontSize',fs);
