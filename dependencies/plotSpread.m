...
%                6: mean +/- 95% confidence standard error: (added by BI)
...
% with group-indices
[m,md,sem,sd,sem95] = deal(nan(max(nData,length(opt.xValues)),1));
...
if opt.showMM
    ...
    switch opt.xyOri
        case 'normal'
            ...
            if opt.showMM == 6
                mh = plot(ah,opt.xValues,m,'+r','Color','r','MarkerSize',12);
                mdh = myErrorbar(ah,opt.xValues,m,sem95);
            end
        case 'flipped'
            if any(opt.showMM==[1,2])
            ...
            if opt.showMM == 6
                mh = plot(ah,opt.xValues,m,'+r','Color','r','MarkerSize',12);
                mdh = myErrorbar(ah,opt.xValues,m,[sem95,NaN(size(sem95))]);
            end
    end
end
